...
\qq To build root, we created a builddir in {\tt /opt/root}, moved into this directory, and used {\tt cmake ..} and {\tt cmake --build} setting the source with {\tt source /opt/root/builddir}. Root can now be run by running the command {\tt root} globally. Now, following the instructions in root's README, we configured root's library with {\tt ./configure linux664gcc}  
